...
function filterPoem(){
    ...
    let displayData = [];
    for (let i=0; i<poem_data.length; i++) {
        if(poem_data[i]['School_e'] == poemStyle_button){
            displayData.push(poem_data[i]);
        }
    }
...
function displayPoem(_data) {
    ...
    // var poemImg = elemEnter.append("svg:img")
    //     .classed("poemImg", true)
    //     .attr("x", function(d,i) { return d['pos_x']+rect_wd-256-buff; })
    //     .attr("y", rect_y+300)
    //     .attr("width", 256)
    ...
    rect
        ...
        .on('click', function(event, d) {
            ...
            // let dataTemp = d3.select(this.parentNode).select('rect')['_groups'][0][0].getBoundingClientRect();
            // console.log(d3.select(this.parentNode).select('.poemImg').data()[0]['Order'])
            // poemImg_id = "img_" + d3.select(this.parentNode).select('.poemImg').data()[0]['Order']
            // poemCurr = d3.select(this.parentNode).select('.poemImg').attr("xlink:href", "img/rural.png");
        })
